...
const reqTypes = config.requestList
//Initialize router
let router = {}
...
router.user.put = (data, callback) => {
    let input = {
        firstName: data.reqBody.firstNameUpdate,
        lastName: data.reqBody.lastNameUpdate,
        phone: data.reqBody.phoneUpdate,
        password: data.reqBody.passOld,
        newPass: data.reqBody.passNew,
        address: data.reqBody.addressUpdate,
        email: data.reqBody.emailUpdate,
        token: data.headers.token
    }
    validator.validateRequest(input, data.method, (reqValid, reqData) => {
        if (reqValid) {
                    ...
                    crud.read('users', reqData.phone, (err, userData) => {
                        ...
                        if (!err && userData && providedPassHash === userData.hashedPass) {
                            ...
                            if(providedPassHash === userData.hashedPass){
                                userData.hashedPass = helpers.hash(reqData.newPass)
                            }
